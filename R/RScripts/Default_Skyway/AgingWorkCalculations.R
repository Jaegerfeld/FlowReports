...
cycleAgingmins <- round((as.numeric(calcDTAging$`In Analysis`)
                    ...
                    + as.numeric(calcDTAging$`E2E Test`)
...
AgingWIPPlot <- ggplot(NotReady_Aging_DT, aes(Status, CycleDays)) + 
  ...
  scale_x_discrete(limits = c("Blocker","Blocked/Waiting", "In Analysis","Backlog","In Implementation", "E2E Test")) +
  ...
  #        x = 1:length( table),
  #        y = aggregate(CycleDays ~ Status, NotReady_Aging_DT, median)[ , 2],
  #        label = c("Blocker","Blocked/Waiting", "In Analysis","Backlog","In Implementation", "E2E Test"),
  #        col = "blue",
  #        size=2,
  #        vjust = 0 )
